...
- **워커 본체는 Vercel Node 단일 런타임**. PRD §8은 "Edge Function/cron 트리거"로 스케치했으나, monolith(네이티브 바이너리 — Deno 서브프로세스 불가)와 Defuddle+JSDOM(Node 전용) 제약으로 편차를 제안했고 **승인됨(2026-07-14, §15)**. 큐 저장·전달 보장은 확정대로 Supabase pgmq.
...
Supabase 자동 백업은 DB만 커버 — Storage(미디어)는 별도. **확정(2026-07-14): Phase 1은 주기적 export 실행으로 갈음** — export zip(미디어 포함)이 백업 산출물을 겸한다. rclone/S3 sync 크론은 제품화 시점 재검토(§15-5).
...
> **2026-07-14 해소 (1~6번 전부)**: ① 임베딩 **gemini-embedding-001**(1536 절단) 확정 ② 워커 **Vercel Node 단일 런타임** 승인 ③ 재저장 = 기존 항목 재캡처(설계안) 확정 ④ X 폴백 병행 호출(설계안) 확정 ⑤ 미디어 백업은 **주기 export로 갈음** ⑥ 파일 크기 상한 제안값 확정. 아래 1~6은 결정 근거 기록용으로 유지.
